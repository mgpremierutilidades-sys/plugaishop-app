...
function Action-CartCrossSellV1() {
  ...
  # Ensure ScrollView import is present
  ...
  # Ensure flag const exists
  if ($after -notmatch 'ff_cart_cross_sell_v1') {
    if ($after -match 'const\s+cartUxUpgrade\s*=\s*isFlagEnabled\("ff_cart_ux_upgrade_v1"\)\s*;') {
      $after = [Regex]::Replace(
        $after,
        'const\s+cartUxUpgrade\s*=\s*isFlagEnabled\("ff_cart_ux_upgrade_v1"\)\s*;\s*',
        'const cartUxUpgrade = isFlagEnabled("ff_cart_ux_upgrade_v1");' + "`r`n" + '  const crossSellV1 = isFlagEnabled("ff_cart_cross_sell_v1");' + "`r`n",
        1
      )
    } elseif ($after -match 'const\s+uiV2\s*=\s*isFlagEnabled\("ff_cart_ui_v2"\)\s*;') {
      $after = [Regex]::Replace(
        $after,
  ...
  # Inject component + footer is brittle by regex; prefer keeping manual paste as source of truth.
  if ($after -ne $before) {
    Set-Content -Path $cartFile -Value $after -Encoding UTF8
    $result.notes += "cart.tsx: applied minimal cross-sell wiring (imports/flag). (UI should be added by the provided full cart.tsx)"
    $result.did_change = $true
  } else {
    $result.notes += "cart.tsx: no changes needed for cart_cross_sell_v1"
  }
}
...
function Action-CartPerformancePassV1() {
  ...
  # Ensure SectionList perf props are present
  if ($after -match '<SectionList[\s\S]*?>' -and $after -notmatch 'removeClippedSubviews') {
    $after = [Regex]::Replace(
      $after,
      '(<SectionList[\s\S]*?stickySectionHeadersEnabled=\{false\}[\s\S]*?)\r?\n(\s*ListFooterComponent=)',
      '${1}' + "`r`n" + '            removeClippedSubviews' + "`r`n" +
  ...
  if ($after -ne $before) {
    ...
    $result.notes += "cart.tsx: no changes needed for cart_performance_pass_v1"
  }
}
