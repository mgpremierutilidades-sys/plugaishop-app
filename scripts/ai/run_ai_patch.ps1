...
function Gates([switch]$Fast) {
  ...
  if ($Fast) {
    ...
    Write-Host "Gates OK (Fast)."
    return
  }

  # Espelho do CI do GitHub (sanity + ci)
  Run-OrFail 'npx expo-doctor'
  Run-OrFail 'npm run ci'
...
try {
  ...
  $patchCmd = 'pwsh -NoProfile -ExecutionPolicy Bypass -File "' + $PatchFile + '" -ProjectRoot "' + $ProjectRoot + '"'
  if ($Fast) { $patchCmd += " -Fast" }
  Run-OrFail $patchCmd
